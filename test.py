...
#genalg_df.to_csv("./algo_results/0_genalg.csv", index= False)
#ant_df.to_csv("./algo_results/0_ant.csv", index= False)
#sm_df.to_csv("./algo_results/0_sm.csv", index= False)
#im_df.to_csv("./algo_results/0_im.csv", index= False)
...
def genalg(genalg_df, size_pop = 50, max_iter = 500, prob_mut = 1):
	...
	genalg_df.to_csv("./algo_results/0_genalg.csv", mode='a', header=False, index= False)
...
def antcolony(ant_df, size_pop=50, max_iter=200):
	...
	ant_df.to_csv("./algo_results/0_ant.csv", mode='a', header=False, index= False)
...
def sim_anealing(sm_df, T_max=100, T_min=1, L=10):
	...
	sm_df.to_csv("./algo_results/0_sm.csv", mode='a', header=False, index= False)
...
def immune(im_df, size_pop=500, max_iter=800, prob_mut=0.2, T=0.7, alpha=0.95):
	...
	sm_df.to_csv("./algo_results/0_im.csv", mode='a', header=False, index= False)
...
for x in range(100):
	...
	for T_max in [250, 500, 1000, 2000, 4000, 8000]:
		for T_min in [1, 5, 10, 25, 50, 125]:
			for L in [5, 10, 25, 50]:
