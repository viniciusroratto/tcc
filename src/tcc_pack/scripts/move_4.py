...
def first_auction (auctioned, some_targets):
	
	target_list = get_points(some_targets)
	uav_list = get_points(uavs)
		
	for xi, x in enumerate(some_targets):
		values = []
...
def fly(xi, x, algo, tick, delivery, prediction, some_targets):	
	size_pop = 26
	max_iter = 10
...
def main():
	...
	some_targets = targets[:number_of_targets]
	global auctioned_targets 
	
	auctioned_targets = first_auction(auctioned_targets, some_targets)
	while (len(auctioned_targets[0]) <= 1 or len(auctioned_targets[1]) <= 1 or len(auctioned_targets[2]) <= 1 or  len(auctioned_targets[3]) <= 1 or len(auctioned_targets[4]) <= 1):
		...
		reset(0, boxes, uavs, some_targets, world_size)
		auctioned_targets = [[],[],[],[],[]]
		auctioned_targets = first_auction(auctioned_targets, some_targets)
	...
	for xi, x in enumerate(uavs):
		...
		try:
			...
			threads.append(_thread.start_new_thread(fly, (xi, x, algo, tick, handover, prediction, some_targets)))
		except:
			print('Flight Over!')
	...
	if(spawn == 'a'):
		for each in range(5):	
			print('Extra:', extra_target)
			rospy.sleep(time/15)
			sem.acquire()
			waiting_list.append([all_targets[extra_target], 6])
			sem.release()
